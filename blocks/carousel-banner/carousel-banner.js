...
export default async function decorate(block) {
  ...
  rows.forEach((row, idx) => {
    ...
    if (slideIndicators) {
      ...
      // Use the second paragraph (subtitle) or first heading as tab label
      const slideContent = slide.querySelector('.carousel-banner-slide-content');
      const tabParagraphs = slideContent ? slideContent.querySelectorAll('p') : [];
      const tabLabel = (tabParagraphs.length > 0 ? tabParagraphs[0].textContent : '') || `Slide ${idx + 1}`;
      indicator.innerHTML = `<button type="button" aria-label="${placeholders.showSlide || 'Show Slide'} ${idx + 1} ${placeholders.of || 'of'} ${rows.length}">${tabLabel}</button>`;
